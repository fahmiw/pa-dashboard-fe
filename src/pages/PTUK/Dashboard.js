...
import { ptukStats, lhpChartData, pengelolaCards } from "./components/constants"; 
...
function Dashboard() {
  ...
  return (
    // Tambahkan padding atas (pt-16) pada mobile agar konten tidak tertutup header yang fixed (jika ada) atau tombol menu
    <div className="bg-[#F8FAFC] min-h-screen font-sans p-4 md:p-8 pb-20 overflow-x-hidden relative"> 
      
      {/* --- HEADER CORRECTION --- */}
      {/* PERBAIKAN UTAMA DISINI: */}
      {/* 1. Gunakan 'flex-row items-center justify-between' SELALU, jangan diubah ke 'flex-col' di mobile. */}
      {/* 2. Tambahkan 'pl-14' atau 'pl-16' pada mobile agar tulisan PTUK tidak menumpuk di bawah tombol menu yang 'fixed'. */}
      <div className="flex flex-row items-center justify-between pl-14 md:pl-0 mb-6 md:mb-10">
        <div className="flex items-center">
            <button 
                onClick={() => setMobileMenuOpen(prev => !prev)}
                // Tombol tetap fixed di pojok kiri atas
            ...
            {/* Ukuran font sedikit disesuaikan di mobile */}
            <h1 className="text-xl md:text-3xl font-bold text-gray-900 tracking-tight">PTUK</h1>
        </div>
      ...
      {/* --- SECTION 1: CHART LHP KEMENTRIAN --- */}
